package helm

import (
	"helmboot/models"
	"helmboot/utils"
	"io/ioutil"
	"log"
	"path"
	"testing"

	"github.com/golang/glog"
	"github.com/stretchr/testify/assert"
	"gopkg.in/yaml.v2"
)

const cmapTestManifest = `
apiVersion: helmboot/beta/v1
type: application
name: myapp
description: "This a deployment of just configmaps"
version: 1.0.0
appVersion: 2.0.1
spec:
  configmaps:
  - appconfig:
      data:
        - mykey: myvalue
  - otherconfig:
      data:
        - key1: value1
        - key2: value2
`

func TestWriteConfigMap(t *testing.T) {
	t.Log("TestWriteConfigMap....")
	// Create the output directory
	dir, err := ioutil.TempDir("", "testing")
	if err != nil {
		log.Fatal(err)
	}
	t.Logf("OutputDir: %v\n", dir)

	var application models.Application
	yamlFile := []byte(cmapTestManifest)
	err = yaml.Unmarshal(yamlFile, &application)
	if err != nil {
		glog.Errorf("Error parsing file: %v", err)
		panic(err)
	}

	metaApp := models.CreateMetaApplication(application)
	WriteConfigmaps(metaApp, dir)

	// Verify file got created.
	filePath := path.Join(dir, "configmaps.yaml")
	t.Logf("Chart content: %s\n", filePath)
	assert.True(t, utils.FileExists(filePath))
}
